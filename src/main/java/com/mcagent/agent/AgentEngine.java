...
import com.mcagent.llm.LlmMessage;
import com.mcagent.llm.ToolCall;
...
public final class AgentEngine {
    ...
    private final ExecutorService executor = Executors.newSingleThreadExecutor(r -> {
        Thread t = new Thread(r, "mcagent-engine");
        t.setDaemon(true);
        return t;
    });
    ...
    private volatile com.mcagent.llm.LlmClient activeClient;
    ...
    public void submit(String userText) {
        ...
        if (!AgentConfig.get().isConfigured()) {
            String msg = I18n.get("mcagent.chat.not_configured");
            if (msg.equals("mcagent.chat.not_configured")) {
    ...
    public void cancel() {
        cancelled = true;
        com.mcagent.llm.LlmClient c = activeClient;
        if (c != null) {
            c.abortInFlight();
        }
    }
        ...
        try {
            ...
            for (int iter = 0; ; iter++) {
                ...
                for (ToolCall call : resp.toolCalls()) {
                    ...
                    com.mcagent.plugin.AgentContext.SHARED.emit("tool.called",
            ...
            com.mcagent.plugin.AgentContext.SHARED.emit("task.finished", userText);
        }
    }
    ...
    private LlmClient.Response request(LlmClient client, ChatModel chat) {
        try {
            try {
                ...
                if (first instanceof com.mcagent.llm.LlmClient.BadRequestException || cancelled
                        || !isTransient(first)) {
                    throw first;
                }
        ...
        } catch (com.mcagent.llm.LlmClient.BadRequestException e) {
            ...
            try {
                ...
                return client.chat(buildRequest(), ToolRegistry.schemas());
            } catch (com.mcagent.llm.LlmClient.BadRequestException e2) {
