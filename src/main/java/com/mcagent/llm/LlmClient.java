...
public final class LlmClient {
    ...
    /**
     * 诊断日志钩子(游戏启动时由 McAgentMod 注入 SLF4J logger,单测环境为 null):
     * 每次 LLM 调用写一行摘要,HTTP 非 2xx 时写完整状态与错误体。
     */
    public static volatile java.util.function.Consumer<String> logHook;

    /** 全链路插桩:每次调用的原始请求/响应字节落盘 config/mcagent/trace/(复现诊断金标准)。 */
    ...
    private static void traceIndex(String line) {
        try {
            java.nio.file.Files.writeString(
                    java.nio.file.Path.of("config", "mcagent", "trace", "index.log"), line + "\n",
    ...
    /** 400 时把被拒的完整请求体转储到 config/mcagent/last_400_request.json(诊断用,失败静默)。 */
    private static void dumpRequestForDebug(JsonObject payload) {
        try {
            java.nio.file.Path p = java.nio.file.Path.of("config", "mcagent", "last_400_request.json");
            java.nio.file.Files.createDirectories(p.getParent());
    ...
    private JsonObject post(JsonObject payload) throws Exception {
        ...
        java.nio.file.Path dir = java.nio.file.Path.of("config", "mcagent", "trace");
        ...
        if (hook != null) {
            if (status == 200) {
                ...
                hook.accept("[llm] #" + seq + " HTTP " + status + "(" + ms + "ms): " + snippet
                        + " ← 完整请求在 config/mcagent/trace/" + seq + "_req.json");
            }
        }
        if (status == 400) {
            String snippet = body.replace('\n', ' ');
