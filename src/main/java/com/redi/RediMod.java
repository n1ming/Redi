...
import com.redi.craft.HudOverlay;
 ...
 */
@Mod(RediMod.MODID)
public final class RediMod {
    public static final String MODID = "redi";
    public static final Logger LOGGER = LoggerFactory.getLogger("redi");
    ...
    public RediMod() {
        ...
        IEventBus modBus = ModList.get().getModContainerById(MODID).orElseThrow(
                () -> new IllegalStateException("redi ModContainer 未就绪")).getEventBus();

        if (FMLEnvironment.dist.isClient()) {
            AgentConfig.get().load();
            // LLM 全链路插桩:每次调用摘要进游戏日志(原始字节在 config/redi/trace/)
            com.redi.llm.LlmClient.logHook = LOGGER::info;
            // 插件装载(cordis 风格:工具/服务/事件全部由插件装配,卸载即回滚)
            com.redi.plugin.PluginManager.loadAll();
            LOGGER.info("[redi] 插件装载完成: {}", com.redi.plugin.PluginManager.loadedIds());
            ...
            modBus.addListener(RediMod::registerCraftHud); // 方法引用:与 ModNetworking::onRegisterPayloads 同款,可稳定编译

            // 客户端游戏总线:捕获指令反馈聊天文本(send_command 用)
            net.neoforged.neoforge.common.NeoForge.EVENT_BUS.addListener(
                    com.redi.tools.CommandFeedback::handleClientChat);
            ...
            net.neoforged.neoforge.common.NeoForge.EVENT_BUS.addListener(
                    (net.neoforged.neoforge.client.event.ClientPlayerNetworkEvent.LoggingOut e)
                            -> com.redi.agent.ChatStore.saveCurrent());
                        ...
                        if (e.getScreen() instanceof net.minecraft.client.gui.screens.PauseScreen) {
                            e.addListener(net.minecraft.client.gui.components.Button.builder(
                                            ...
                                            b -> e.getScreen().getMinecraft().setScreen(
                                                    new com.redi.client.AgentPluginsScreen(e.getScreen())))
                                    .bounds(8, 8, 96, 20)
                                    .build());
                        }
                    });

            LOGGER.info("[redi] Redi App 已就绪(在手机主屏打开「Redi」)");
        }
        ...
        LOGGER.info("[redi] 服务端基建已注册(虚拟工作台菜单 / 虚拟熔炉会话 / 冶炼同步)");
    }
