...
public final class AgentFlatScreen extends Screen {
    ...
    protected void init() {
        ...
        if (!autoRestoredLaunch) {
            ...
            if (com.redi.agent.ChatModel.get().snapshot().isEmpty()) {
                com.redi.agent.ChatStore.loadLatest();
            }
        }
    ...
    public void removed() {
        ...
        com.redi.RediMod.LOGGER.info("[redi] 平铺 removed(关闭/切屏)");
        com.redi.agent.ChatStore.saveCurrent(); // 关屏自动存档(空会话不落盘)
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (nav != null && nav != com.november.mcphone.core.client.PhoneChassis.NavButton.NONE) {
            com.redi.RediMod.LOGGER.info("[redi] 导航栏点击: {}", nav);
